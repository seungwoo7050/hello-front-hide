    ...
    it('validateForm으로 전체 폼을 검사할 수 있다', () => {
      const { result } = renderHook(() =>
        useForm({
          ...
          validationRules: {
            name: [
              validators.required<{ name: string; email: string }>('이름 필수'),
            ],
            email: [
              validators.required<{ name: string; email: string }>(
                '이메일 필수'
              ),
            ],
          },
...
describe('validators', () => {
  describe('required', () => {
    it('빈 값에 에러를 반환한다', () => {
      ...
      expect(rule.validate('   ', {} as { field: string })).toBe(false)
      expect(
        rule.validate(null as unknown as string, {} as { field: string })
      ).toBe(false)
      expect(
        rule.validate(undefined as unknown as string, {} as { field: string })
      ).toBe(false)
  ...
  describe('email', () => {
    ...
    it('이메일 형식이면 true를 반환한다', () => {
      const rule = validators.email<{ field: string }>('이메일 형식 오류')
      expect(rule.validate('test@example.com', {} as { field: string })).toBe(
        true
      )
    })
  ...
  describe('match', () => {
    it('값이 일치하지 않으면 false를 반환한다', () => {
      const rule = validators.match<{
        password: string
        confirmPassword: string
      }>('password', '비밀번호 불일치')
      expect(
        rule.validate('different', {

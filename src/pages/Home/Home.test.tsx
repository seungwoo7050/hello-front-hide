...
describe('Home', () => {
  it('히어로 섹션을 렌더링한다', () => {
    ...
    expect(
      screen.getByRole('heading', { name: /Hello Front/i })
    ).toBeInTheDocument()
  })
  ...
  it('히어로 설명을 표시한다', () => {
    renderWithRouter(<Home />)
    expect(
      screen.getByText(/React \+ TypeScript 학습 여정/i)
    ).toBeInTheDocument()
  })
  ...
  it('CTA 버튼들을 렌더링한다', () => {
    renderWithRouter(<Home />)
    expect(
      screen.getByRole('link', { name: /Playground 둘러보기/i })
    ).toBeInTheDocument()
    expect(
      screen.getByRole('link', { name: /프로젝트 소개/i })
    ).toBeInTheDocument()
  })

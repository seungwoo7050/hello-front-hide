...
describe('FormDemo', () => {
  it('페이지 제목을 렌더링한다', () => {
    ...
    expect(
      screen.getByRole('heading', { name: /Form \+ Validation/i, level: 1 })
    ).toBeInTheDocument()
  })
  ...
  it('토스트 알림 섹션을 렌더링한다', () => {
    ...
    expect(
      screen.getByRole('heading', { name: /Toast 알림/i, level: 2 })
    ).toBeInTheDocument()
  ...
  it('문의 폼 섹션을 렌더링한다', () => {
    ...
    expect(
      screen.getByRole('heading', { name: /문의 폼/i, level: 2 })
    ).toBeInTheDocument()
  })
  ...
  it('토스트 버튼 클릭 시 토스트가 표시된다', async () => {
    ...
    expect(
      screen.getByText('작업이 성공적으로 완료되었습니다.')
    ).toBeInTheDocument()
  })
  ...
  it('카테고리 선택 옵션들이 있다', () => {
    ...
    expect(
      screen.getByRole('option', { name: '일반 문의' })
    ).toBeInTheDocument()
    expect(
      screen.getByRole('option', { name: '기술 지원' })
    ).toBeInTheDocument()
    expect(screen.getByRole('option', { name: '피드백' })).toBeInTheDocument()
    expect(
      screen.getByRole('option', { name: '제휴 문의' })
    ).toBeInTheDocument()
  })

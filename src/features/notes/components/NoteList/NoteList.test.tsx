...
describe('NoteList', () => {
  ...
  it('필터가 활성화되면 초기화 버튼을 표시한다', () => {
    ...
    render(<NoteList {...defaultProps} filter={filterWithSearch} onClearFilter={onClearFilter} />);
  ...
  it('필터 적용 후 노트가 없으면 다른 메시지를 표시한다', () => {
    const filterWithSearch: NotesFilter = { ...defaultFilter, search: 'nonexistent' };
  ...
  it('선택된 노트를 표시한다', () => {
    ...
    const { container } = render(<NoteList {...defaultProps} selectedNote={selectedNote} />);

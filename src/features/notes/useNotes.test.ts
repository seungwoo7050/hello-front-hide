...
describe('useNotes', () => {
  ...
  describe('updateNote', () => {
    it('노트를 업데이트한다', () => {
      ...
      const updatedNote = result.current.notes.find(n => n.id === firstNote.id);
  ...
  describe('deleteNote', () => {
    it('노트를 삭제한다', () => {
      ...
      expect(result.current.notes.length).toBe(initialCount - 1);
      expect(result.current.notes.find(n => n.id === firstNote.id)).toBeUndefined();
    });
  ...
  describe('togglePin', () => {
    it('노트를 고정/해제한다', () => {
      ...
      const toggledNote = result.current.notes.find(n => n.id === unpinnedNote.id);
      ...
      const toggledAgain = result.current.notes.find(n => n.id === unpinnedNote.id);
  ...
  describe('필터링', () => {
    it('검색어로 필터링한다', () => {
      ...
      expect(result.current.filteredNotes.every(n => 
